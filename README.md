...
Because the generated HTML, CSS, JavaScript, and search index span tens of thousands of lines, committing the built `site/` directory makes pull requests too large to review or even open. Instead, the updated workflow builds the site for every change and, on pushes to `main`, publishes it directly to the `gh-pages` branch using GitHub Pages. Pull requests still receive a build verification artifact so reviewers can download the rendered documentation without bloating the repository history.【F:.github/workflows/build-mkdocs.yml†L1-L61】
