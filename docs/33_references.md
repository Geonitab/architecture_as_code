# References and Sources {#appendix-references}

This section provides a comprehensive list of all sources and references cited throughout the book, compiled in alphabetical order for ease of reference. Each entry includes information about which chapters reference the source.

## Numbered source index

### Source [4]: GitHub Docs. *About protected branches.* GitHub Documentation, 2024. {#source-4}
Referenced in: [Chapter 03: Version Control and Code Structure][chapter-version-control], [Chapter 11: Governance as Code][chapter-governance-as-code], [Chapter 14: Practical Implementation][chapter-practical-implementation], [Chapter 23: Software as Code Interplay][chapter-soft-as-code-interplay].
### Source [7]: Cloud Native Computing Foundation. *State of Cloud Native Development 2024.* Cloud Native Computing Foundation, 2024. {#source-7}
Referenced in: [Chapter 7: Containerisation and Orchestration as Code][chapter-containerisation].
### Source [8]: Google Cloud. *The Site Reliability Workbook: Practical Ways to Implement SRE.* O'Reilly Media, 2018. {#source-8}
Referenced in: [Chapter 05: Automation, DevOps and CI/CD][chapter-automation-devops-cicd].
### Source [9]: AWS. *AWS Cloud Development Kit (CDK) Developer Guide.* Amazon Web Services, 2024. {#source-9}
Referenced in: [Chapter 03: Version Control and Code Structure][chapter-version-control], [Chapter 05: Automation, DevOps and CI/CD][chapter-automation-devops-cicd], [Chapter 14: Practical Implementation][chapter-practical-implementation].
### Source [12]: Microsoft Learn. *Design multi-stage release pipelines with approvals.* Microsoft Learn Documentation, 2024. {#source-12}
Referenced in: [Chapter 05: Automation, DevOps and CI/CD][chapter-automation-devops-cicd].
### Source [15]: Pulumi. *Testing Infrastructure as Code Programs.* Pulumi Blog, 2024. {#source-15}
Referenced in: [Chapter 13: Testing Strategies for Infrastructure as Code][chapter-testing-strategies].
### Source [16]: HashiCorp. *Securing Terraform State.* HashiCorp Developer Documentation, 2024. {#source-16}
Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Advanced Security Patterns and Implementation][chapter-advanced-security-patterns].
### Source [17]: HashiCorp. *Backend Type: s3.* HashiCorp Developer Documentation, 2024. {#source-17}
Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Advanced Security Patterns and Implementation][chapter-advanced-security-patterns].
### Source [18]: Microsoft Learn. *Store Terraform state in Azure Storage.* Microsoft Learn Documentation, 2024. {#source-18}
Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Advanced Security Patterns and Implementation][chapter-advanced-security-patterns].
### Source [19]: Google Cloud. *Store Terraform state in Cloud Storage.* Google Cloud Documentation, 2024. {#source-19}
Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Advanced Security Patterns and Implementation][chapter-advanced-security-patterns].
### Source [20]: HashiCorp. *Terraform Security Best Practices.* HashiCorp Learning Resources, 2023. {#source-20}
Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Advanced Security Patterns and Implementation][chapter-advanced-security-patterns].

## Academic and Industry Publications

**AaC Open Source Project.** "Architecture-as-Code Repository." [https://github.com/aacplatform/aac](https://github.com/aacplatform/aac)
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr], [Appendix B: Technical Architecture for Book Production][appendix-technical-architecture]*

**Architecture Decision Records Community.** "ADR Guidelines and Templates." [https://adr.github.io](https://adr.github.io)  
*Referenced in: [Chapter 4: Architecture Decision Records][chapter-architecture-decision-records]*

**Atlassian.** "Documentation as Code: Treating Docs as a First-Class Citizen." Atlassian Developer, 2023.
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Atlassian.** "Git Workflows for Architecture as Code." Atlassian Git Documentation.
*Referenced in: [Chapter 3: Version Control][chapter-version-control]*

**Brown, S.** "C4 Model Overview." [https://c4model.com/](https://c4model.com/)  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr], [Chapter 24: Best Practices][chapter-best-practices]*

**Brown, S.** "Documenting Software Architecture with Structurizr." Structurizr Blog, 2022.  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr], [Chapter 24: Best Practices][chapter-best-practices]*

**Brown, S.** "Software Architecture for Developers." Leanpub, 2024.  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr]*

**Chung, L., et al.** "Non-Functional Requirements in Software Engineering." Springer, 2000.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Cloud Native Computing Foundation.** "State of Cloud Native Development 2024." Cloud Native Computing Foundation, 2024. (Source [7])
*Referenced in: [Chapter 1: Introduction][chapter-introduction], [Chapter 7: Containerisation][chapter-containerisation]*

**FINOS.** "CALM: Common Architecture Language Model." FINOS Architecture as Code Community, 2024. [https://calm.finos.org/](https://calm.finos.org/)  
*Referenced in: [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**Ford, Neal et al.** "Building Evolutionary Architectures." O'Reilly Media, 2017.  
*Referenced in: [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**Forsberg, K., Mooz, H.** "The Relationship of System Engineering to the Project Cycle." Engineering Management Journal, 1991.
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**GitLab.** "Documentation as Code: Best Practices and Implementation." GitLab Documentation, 2024.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**GitLab.** "Building a Single Source of Truth with APIs and CLI." GitLab Topics, 2024.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles], [Chapter 11: Governance as Code][chapter-governance-as-code]*

**GitHub Docs.** "About protected branches." [https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/about-protected-branches](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/about-protected-branches)
*Referenced in: [Chapter 03: Version Control and Code Structure][chapter-version-control], [Chapter 11: Governance as Code][chapter-governance-as-code], [Chapter 14: Practical Implementation][chapter-practical-implementation], [Chapter 23: Software as Code Interplay][chapter-soft-as-code-interplay]*

**GitHub Open Source Community.** "Collaborative Software Development." Platform Documentation, 2024.
*Referenced in: [Chapter 29: About the Authors][appendix-about-the-authors]*

**IEEE.** "IEEE Standard for Software Verification and Validation." IEEE Std 1012-2016, 2017.
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**HashiCorp.** "Policy as Code Overview." [https://developer.hashicorp.com/terraform/enterprise/policy-as-code](https://developer.hashicorp.com/terraform/enterprise/policy-as-code)  
*Referenced in: [Chapter 23: Software as Code Interplay][chapter-soft-as-code-interplay]*

**Kvadrat AB.** "Gunnar Nordqvist - Chief Architect Profile." Konsultprofil, 2024. Available at: [https://www.kvadrat.se/anlita-kvadrat/hitta-konsult/gunnar-nordqvist/](https://www.kvadrat.se/anlita-kvadrat/hitta-konsult/gunnar-nordqvist/)  
*Referenced in: [Chapter 29: About the Authors][appendix-about-the-authors]*

**Kvadrat AB.** "Technology Consulting Excellence." Company Profile, 2024. Available at: [https://www.kvadrat.se/](https://www.kvadrat.se/)  
*Referenced in: [Chapter 29: About the Authors][appendix-about-the-authors]*

**MarketsandMarkets.** "Infrastructure as Code Market Report." MarketsandMarkets, 2023.  
*Referenced in: [Chapter 15: Cost Optimisation][chapter-cost-optimisation], [Chapter 25: Future Trends Development][chapter-future-trends]*

**Martin, R.** "Clean Architecture: A Craftsman's Guide to Software Structure." Prentice Hall, 2017.  
*Referenced in: [Chapter 1: Introduction][chapter-introduction], [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Mermaid.** "Mermaid: Diagramming and Charting Tool." Mermaid Documentation, 2024. [https://mermaid.js.org/](https://mermaid.js.org/)  
*Referenced in: [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**NIST.** "Requirements Engineering for Secure Systems." NIST Special Publication 800-160, 2023.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Nygard, M.** "Documenting Architecture Decisions." 2011.  
*Referenced in: [Chapter 4: Architecture Decision Records][chapter-architecture-decision-records]*

**Open Policy Agent.** "Policy as Code: Expressing Requirements as Code." CNCF OPA Project, 2024.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Open Policy Agent.** "Policy as Code Overview." [https://www.openpolicyagent.org/docs/latest/](https://www.openpolicyagent.org/docs/latest/)  
*Referenced in: [Chapter 11: Governance as Code][chapter-governance-as-code], [Chapter 23: Software as Code Interplay][chapter-soft-as-code-interplay]*

**OMG.** "Model Driven Architecture (MDA)." Object Management Group White Paper, 2001.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**PlantUML.** "PlantUML: Open-source Tool for Creating UML Diagrams." PlantUML Documentation, 2024. [https://plantuml.com/](https://plantuml.com/)  
*Referenced in: [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**Red Hat.** "Architecture as Code Principles and Best Practices." Red Hat Developer.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Richardson, C.** "Microservices Patterns: With Examples in Java." Manning Publications, 2018.  
*Referenced in: [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**Schmidt, D. C.** "Model-Driven Engineering." IEEE Computer, 2006.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles]*

**Selic, B.** "The Pragmatics of Model-Driven Development." IEEE Software, 2003.  
*Referenced in: [Chapter 2: Fundamental Principles][chapter-fundamental-principles], [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr], [Chapter 23: Software as Code Interplay][chapter-soft-as-code-interplay]*

**Structurizr.** "Structurizr DSL Language Reference." Structurizr Documentation, 2024. [https://github.com/structurizr/dsl](https://github.com/structurizr/dsl)  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr], [Chapter 22: Documentation as Code vs Architecture as Code][chapter-documentation-vs-architecture]*

**Structurizr.** "Structurizr Lite." Structurizr Documentation, 2024. [https://structurizr.com/help/lite](https://structurizr.com/help/lite)  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr]*

**ThoughtWorks Technology Radar.** "Diagrams as Code." [https://www.thoughtworks.com/radar/techniques/diagrams-as-code](https://www.thoughtworks.com/radar/techniques/diagrams-as-code)  
*Referenced in: [Chapter 6: Structurizr and Diagram Automation][chapter-structurizr]*

### **ThoughtWorks.** "Architecture as Code: The Next Evolution." Technology Radar, 2024. {#thoughtworks-architecture-as-code-the-next-evolution}
*Referenced in: [Chapter 1: Introduction][chapter-introduction], [Chapter 2: Fundamental Principles][chapter-fundamental-principles], [Chapter 29: About the Authors][appendix-about-the-authors]*

**ThoughtWorks.** "Architecture Decision Records." Technology Radar, 2023.  
*Referenced in: [Chapter 4: Architecture Decision Records][chapter-architecture-decision-records]*

**Thoughtworks Technology Radar.** "Governance as Code." [https://www.thoughtworks.com/radar/techniques/governance-as-code](https://www.thoughtworks.com/radar/techniques/governance-as-code)  
*Referenced in: [Chapter 11: Governance as Code][chapter-governance-as-code]*

## Industry Research and Reports

**AWS.** "AWS Cloud Development Kit (CDK) Developer Guide." [https://docs.aws.amazon.com/cdk/latest/guide/home.html](https://docs.aws.amazon.com/cdk/latest/guide/home.html)
*Referenced in: [Chapter 03: Version Control and Code Structure][chapter-version-control], [Chapter 5: Automation, DevOps, and CI/CD][chapter-automation-devops-cicd], [Chapter 14: Practical Implementation][chapter-practical-implementation]*

**Best practice documentation from leading organisations.**  
*Referenced in: [Chapter 27: Conclusion][chapter-conclusion]*

**Expert interviews and case studies.**  
*Referenced in: [Chapter 27: Conclusion][chapter-conclusion]*

**Gartner.** "Forecast Analysis: Public Cloud Services Worldwide." Gartner Research, 2024.  
*Referenced in: [Chapter 15: Cost Optimisation][chapter-cost-optimisation], [Chapter 17: Organisational Change][chapter-organisational-change]*

**Industry reports on Architecture as Code adoption trends.**  
*Referenced in: [Chapter 27: Conclusion][chapter-conclusion]*

**IDC.** "Worldwide DevOps Software Tools Forecast, 2023–2027." IDC Research, 2023.  
*Referenced in: [Chapter 5: Automation, DevOps, and CI/CD][chapter-automation-devops-cicd], [Chapter 21: Digitalisation][chapter-digitalisation]*

**Research on emerging technologies.**  
*Referenced in: [Chapter 27: Conclusion][chapter-conclusion]*

**Google Cloud.** "Store Terraform state in Cloud Storage." Google Cloud Documentation, 2024. [https://cloud.google.com/docs/terraform/resource-management/store-terraform-state](https://cloud.google.com/docs/terraform/resource-management/store-terraform-state) (Source [19])
*Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Security Patterns and Implementation][chapter-advanced-security-patterns]*

**HashiCorp.** "Backend Type: s3." HashiCorp Developer Documentation, 2024. [https://developer.hashicorp.com/terraform/language/settings/backends/s3](https://developer.hashicorp.com/terraform/language/settings/backends/s3) (Source [17])
*Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Security Patterns and Implementation][chapter-advanced-security-patterns]*

### **HashiCorp.** "Securing Terraform State." HashiCorp Developer Documentation, 2024. [https://developer.hashicorp.com/terraform/cloud-docs/state/securing](https://developer.hashicorp.com/terraform/cloud-docs/state/securing) (Source [16]) {#hashicorp-securing-terraform-state}
*Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Security Patterns and Implementation][chapter-advanced-security-patterns]*

**HashiCorp.** "State of Cloud Strategy Survey 2024." HashiCorp, 2024.  
*Referenced in: [Chapter 17: Organisational Change][chapter-organisational-change], [Chapter 27: Conclusion][chapter-conclusion]*

**HashiCorp.** "Terraform Security Best Practices." HashiCorp Learning Resources, 2023. [https://developer.hashicorp.com/terraform/cloud-docs/recommended-practices/security](https://developer.hashicorp.com/terraform/cloud-docs/recommended-practices/security) (Source [20])
*Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Security Patterns and Implementation][chapter-advanced-security-patterns]*

**Microsoft Learn.** "Store Terraform state in Azure Storage." Microsoft Learn Documentation, 2024. [https://learn.microsoft.com/en-gb/azure/developer/terraform/store-state-in-azure-storage](https://learn.microsoft.com/en-gb/azure/developer/terraform/store-state-in-azure-storage) (Source [18])
*Referenced in: [Chapter 9: Security Fundamentals for Architecture as Code][chapter-security-fundamentals], [Chapter 9b: Security Patterns and Implementation][chapter-advanced-security-patterns]*

**Pulumi.** "Testing Infrastructure as Code Programs." Pulumi Blog, 2024. (Source [15])
*Referenced in: [Chapter 13: Testing Strategies][chapter-testing-strategies], [Chapter 14: Practical Implementation][chapter-practical-implementation]*

## Using This References Section

This comprehensive list ensures traceability between the book's content and its sources. When reading a chapter:

1. Check the chapter's "Sources" section for immediate context
2. Refer to this comprehensive list for full bibliographic details
3. Use the cross-references to find related discussions across chapters

All citations in the manuscript are included in this compiled list, ensuring complete transparency and academic rigour throughout the work.
