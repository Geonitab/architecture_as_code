...
Large-scale programmes quickly learned that full-code generation created unsustainable feedback loops. Selic (2003) and Hailpern and Tarr (2006) describe how brittle round-trip engineering, tool lock-in, and ambiguous semantics caused models to diverge as soon as teams customised generated code or adopted new frameworks. The clean separation between models and implementation rarely survived the first production incident. Selic's historical review emphasised that the few durable MDD successes constrained automation to well-defined domains, invested in formal semantics, and accepted that human-crafted code would continue to carry the complex business logic.

Architecture as Code internalises those lessons by favouring lightweight textual models, explicit version control, and continuous validation so that the architecture specification evolves alongside implementation rather than trying to replace it. Modern MBSE-aligned tooling such as Structurizr applies that pragmatic guidance by generating diagrams and review artefacts from a single authoritative model while leaving engineers in control of the underlying code and transformations. Its DSL-first approach keeps models close to the technology teams who curate them, echoing Selic's recommendation to use precise modelling languages and resist promises of universal round-trip generation. This approach provides the automation benefits MDD aspired to while preserving developer autonomy, making Architecture as Code a practical successor that avoids the historical pitfalls identified in the early 2000s.
