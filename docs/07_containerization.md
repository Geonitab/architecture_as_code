...
### Codifying Governance for Regulated Workloads

Klarna's regulated microservices, preserved in the archive, illustrate why container platforms without Architecture as Code become unmanageable. Each payment service must demonstrate compliance with Finansinspektionen, PCI-DSS, PSD2, and AML/KYC simultaneously, whilst carrying seven years of immutable audit data and sustainability reporting. Those obligations now live inside Kubernetes manifests, Helm charts, and GitOps pipelines: registry policies enforce encryption, operators encode data-retention periods, and Argo CD applications embed risk classifications directly into deployment metadata. When squads adjust routing to satisfy latency targets or energy-optimised scheduling, the same pull request updates carbon telemetry and compliance controls so auditors can trace intent. In short, container estates that run Swedish microservices need Architecture as Code to keep governance, observability, and sustainability tied to every release.
